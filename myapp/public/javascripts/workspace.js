...
function snap_box_to_exg(px_str, callback) {
    ...
    let box_max_x = box_min_x + box_w;
    let box_max_y = box_min_y + box_h;
    ...
    rgb_image.onload = function() {
        ...
        rgb_ctx.drawImage(rgb_image, 0, 0, w, h);
        d_rgb = rgb_ctx.getImageData(0, 0, w, h);
        ...
        let min_exg_x = max_canvas_x;
        let min_exg_y = max_canvas_y;
        let max_exg_x = min_canvas_x;
        let max_exg_y = min_canvas_y;

        for (let i = min_canvas_y; i < max_canvas_y; i++) {
            for (let j = min_canvas_x; j < max_canvas_x; j++) {
                ...
                if (is_foreground) {
                    foreground_xs.push(j);
                    foreground_ys.push(i);
        ...
        if ((min_exg_x >= max_exg_x) || (min_exg_y >= max_exg_y)) {
            ...
            box_min_x = min_box_pt.x;
            box_min_y = min_box_pt.y;
    
            box_max_x = max_box_pt.x;
            box_max_y = max_box_pt.y;
        }
